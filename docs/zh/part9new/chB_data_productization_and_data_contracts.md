...
### B.1.3 数据产品画布：输入、输出、SLA、Owner 与变更策略
...
如图 B-1 所示，数据产品画布把一个数据产品的关键要素组织在一张视图中，使生产者、消费者与治理团队能够在同一语境下讨论这个产品的边界与承诺。

![数据产品画布](../../images/part9new/图B_1zh.png)
...
一份完整的数据契约通常包含五类核心条款，分别覆盖数据可靠性的不同侧面。表 B-2 概括了这五类契约的关注问题与典型条款。
...
把上述五类条款组织在一起，就构成一份完整的数据契约。如图 B-2 所示，一份数据契约模板通常包含元信息（名称、版本、Owner）、五类条款，以及契约的生效与变更信息。

![Data Contract 模板：五类条款的结构](../../images/part9new/图B_2zh.png)
...
并非所有变更都同等危险。按对下游的影响，数据变更可以分为三类，如表 B-3 所示。
...
为了在工程实践中快速、一致地判断一个变更应当如何处理，可以使用一棵**变更兼容性决策树**，如图 B-3 所示。

![变更兼容性决策树](../../images/part9new/图B_3zh.png)
...
这次事故清晰地暴露了沉默失败的代价。如果当时具备消费者注册表与影响分析能力，上游在提出变更时，就能立即得到一张受影响消费者的清单。表 B-4 展示了这样一份事后补做的消费者影响分析。

*表 B-4：消费者影响分析表（`interaction_type` 枚举拆分）*
...
这张表把一次变更的影响从模糊的担忧，变成了可逐行评估的清单：谁受影响、怎么受影响、有多严重、该怎么应对。其中 `analytics_daily` 这类虽受影响但后果较轻的消费者也被纳入，体现了影响分析的完整性。如图 B-4 所示，这种分析的本质，是沿着血缘把一次上游变更映射到所有下游消费者，并按影响严重程度分级。

![消费者影响分析](../../images/part9new/图B_4zh.png)
